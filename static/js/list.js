...
var socket = io.connect('//'+window.location.hostname+':3000', {'force new connection': true});
	...
	socket.on("get_list", function(){
		...
		console.log("inni socket on", chan);
		socket.emit('list', chan+','+guid);
	});
$("#chan").html((window.location.pathname.split("/")[1]).charAt(0).toUpperCase() + (window.location.pathname.split("/")[1]).slice(1));
var chan = $("#chan").html();
